...
# Task22 ON canary:
#   partial-rollout tail hedging + sync-intent adaptive window + old-debt priority.
...
[[ -z "$(git -C "$REPO" status --porcelain=v1 --untracked-files=all)" ]] || {
    echo "Task22 ON canary requires a clean worktree" >&2
    exit 4
}
...
GPU_MONITOR_PATH="$TASK22_LOCAL_RUN_DIR/nvidia_smi_1s.csv"
GPU_MONITOR_PID=""
...
export RELAX_TASK22_SYNC_INTENT_GUARD="1"
export RELAX_TASK22_SYNC_INTENT_TTL_SECONDS="600"
export RELAX_TASK22_ADAPTIVE_OVERSAMPLING_GROUPS="16"
...
[[ "$NUM_ROLLOUT" == "16" ]] || { echo "NUM_ROLLOUT must be 16" >&2; exit 4; }
WORKING_DIR="$REPO"
...
TASK22_PYTHON="$PYTHON_BIN" MODEL_DIR="$MODEL_DIR" DATA_DIR="$DATA_DIR" \
    "$REPO/scripts/task22/preflight_sync_intent_priority.sh" --formal \
...
for name in (
    ...
    "RELAX_TASK22_SYNC_INTENT_TTL_SECONDS",
    "RELAX_TASK22_ADAPTIVE_OVERSAMPLING_GROUPS",
):
...
METRIC_ARGS=(
    --use-clearml
    --use-metrics-service
    --tb-project-name "Relax/task22/sync-intent-priority"
    --tb-experiment-name "task22-sync-intent-priority-$(date '+%Y%m%d-%H%M%S')"
)
MISC_ARGS=(
...
payload = {
    ...
    "upstream_main_base_sha": "0bc99af8dd39de8fd99c588a98b3f3a463bc818c",
    "python": os.environ["TASK22_PYTHON"],
    "headline": {"logical_step_lo": 5, "logical_step_hi": 14},
    "measurement_contract": {
        "purpose": "Task22 sync-intent adaptive oversampling and old-debt priority ON canary",
        "historical_wall_directly_comparable": False,
        "reason": "1s scheduler/permit/timing instrumentation enabled; historical timeline dump intentionally omitted",
    },
    ...
    "workload": {
        "num_rollout": int(os.environ.get("NUM_ROLLOUT", "16")),
        ...
        "sync_intent_guard": True,
        "old_debt_priority": True,
        "rollout_seed": 42,
        "train_seed": 1234,
    },
...
source_paths = {
    ...
    "relax_rollout_service": repo / "relax/components/rollout.py",
    "relax_rollout_manager": repo / "relax/distributed/ray/rollout.py",
}
try:
