...
if [[ "${TASK22_FAST_PREFLIGHT:-0}" != "1" ]]; then
    "$PYTHON_BIN" -m pytest -q \
        ...
        tests/utils/test_hybrid_dcs_minimal_contract_source.py \
...
if [[ "${TASK22_EXPERIMENT_ARM:-baseline}" == "dcs_joint_on" || "${TASK22_EXPERIMENT_ARM:-baseline}" == "dcs_a3_minimal_on" ]]; then
    "$PYTHON_BIN" - <<'PY'
