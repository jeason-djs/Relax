...
def _validate_run_artifacts(
    ...
    *,
    ...
    if (headline_lo, headline_hi) != (5, 14):
        raise CalibrationInputError(
            f"invalid_calibration_headline_contract:{headline_lo}..{headline_hi}:expected=5..14"
        )
    ...
    if num_rollout != 16:
        raise CalibrationInputError(f"invalid_run_contract_num_rollout:{num_rollout!r}:expected=16")
    contract_headline = contract.get("headline")
    expected_headline = {"logical_step_lo": 5, "logical_step_hi": 14}
...
def _validate_sglang_timing(row: dict[str, Any]) -> bool:
    if row.get("permit_wait_status") != "terminal" or row.get("exception_type") is not None:
        return False
...
def _sglang_timing_group_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    eligible = [row for row in terminal if row.get("exception_type") is None]
    ...
    return {
        ...
        "terminal_with_exception_excluded_count": len(terminal) - len(eligible),
    ...
    return {
        ...
        "semantics": {
            ...
            "sglang_queue_prefill_timing": (
                ...
                "used. Complete tuples are validated and summarized by fresh/resume. Every physical rollout 5..14 "
                "terminal request with exception_type=None requires complete timing (100% coverage); local exceptions and "
