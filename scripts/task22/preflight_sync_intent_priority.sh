...
EXPECTED_PATHS=(
    docs/draft/task22_sync_intent_adaptive_oversampling.md
    ...
    relax/engine/rollout/sync_intent.py
    ...
    scripts/task22/preflight_sync_intent_priority.sh
    ...
    scripts/task22/run_sync_intent_priority_on.sh
...
[[ -z "$(git status --porcelain=v1 --untracked-files=all)" ]] || {
    echo "Task22 sync-intent preflight requires a clean worktree" >&2
    exit 4
}
git merge-base --is-ancestor "$BASE_SHA" HEAD || {
    echo "Task22 ON HEAD does not contain upstream/main base $BASE_SHA" >&2
    exit 4
}
[[ -z "$(git rev-list --merges "$BASE_SHA"..HEAD)" ]] || {
    echo "Task22 ON branch must not contain merge commits" >&2
    exit 4
}
diff -u \
    <(printf '%s\n' "${EXPECTED_PATHS[@]}" | LC_ALL=C sort) \
    <(git diff --name-only "$BASE_SHA" HEAD | LC_ALL=C sort) || {
    echo "Task22 ON diff contains a missing or unexpected path" >&2
    exit 4
}
...
"$PYTHON_BIN" -m py_compile \
    ...
    scripts/task22/analyze_phase_elastic_calibration.py \
    scripts/task22/smoke_sglang_calibration.py
bash -n scripts/task22/run_sync_intent_priority_on.sh
...
if [[ "$MODE" == "--local" ]]; then
    echo "TASK22_SYNC_INTENT_PREFLIGHT verdict=PASS_LOCAL"
    exit 0
fi
...
[[ "${RELAX_TASK22_SYNC_INTENT_GUARD:-}" == "1" ]] || {
    echo "Formal preflight requires RELAX_TASK22_SYNC_INTENT_GUARD=1" >&2
    exit 4
}
[[ "${RELAX_TASK22_ADAPTIVE_OVERSAMPLING_GROUPS:-}" == "16" ]] || {
    echo "Formal preflight requires RELAX_TASK22_ADAPTIVE_OVERSAMPLING_GROUPS=16" >&2
    exit 4
}
...
print("TASK22_SYNC_INTENT_PREFLIGHT gpu_contract=4x_RTX_PRO_6000_96GB_PASS")
PY
"$PYTHON_BIN" - <<'PY'
import torch
...
print("TASK22_SYNC_INTENT_PREFLIGHT flashinfer_arch=SM120a_RMSNORM_PASS")
PY
...
print(f"TASK22_SYNC_INTENT_PREFLIGHT relax_origin={origin}")
PY
"$PYTHON_BIN" - <<'PY'
import inspect
...
print("TASK22_SYNC_INTENT_PREFLIGHT megatron_dgrad_fold=PASS")
PY
...
echo "TASK22_SYNC_INTENT_PREFLIGHT gpu_count=$gpu_count"
echo "TASK22_SYNC_INTENT_PREFLIGHT verdict=PASS_FORMAL"
