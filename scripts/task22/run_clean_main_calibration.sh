...
elif [[ "$TASK22_EXPERIMENT_ARM" == "dcs_a3_minimal_on" ]]; then
    SGLANG_ARGS+=(
        --use-slime-router
    )
fi
...
if [[ "$TASK22_EXPERIMENT_ARM" == "a3_workaware_on" || "$TASK22_EXPERIMENT_ARM" == "dcs_joint_on" || "$TASK22_EXPERIMENT_ARM" == "dcs_a3_minimal_on" ]]; then
    TRAIN_ARGS+=(
        --enable-cross-version-kv-continuation
        --cross-version-kv-max-gap 2
    )
fi
if [[ "$TASK22_EXPERIMENT_ARM" == "dcs_joint_on" || "$TASK22_EXPERIMENT_ARM" == "dcs_a3_minimal_on" ]]; then
    TRAIN_ARGS+=(
...
payload = {
    ...
    "measurement_contract": {
        "purpose": (
            ...
            if os.environ["TASK22_EXPERIMENT_ARM"] == "a3_workaware_on"
                ...
                "zero-KL request-level A3 + targeted retirement + in-place Hybrid DCS ON; "
                "default least-loaded routing without Admission, priority, or work-aware placement"
                if os.environ["TASK22_EXPERIMENT_ARM"] == "dcs_a3_minimal_on"
                else (
    ...
    "workload": {
        ...
        in {"a3_workaware_on", "dcs_joint_on", "dcs_a3_minimal_on"},
        ...
        "slime_router_default_least_loaded": os.environ["TASK22_EXPERIMENT_ARM"] == "dcs_a3_minimal_on",
        "hybrid_dcs_weight_sync": os.environ["TASK22_EXPERIMENT_ARM"]
        in {"dcs_joint_on", "dcs_a3_minimal_on"},
