...
def validate_contract(runtime: Any, argv: Any) -> dict[str, Any]:
    ...
    if arm not in {"baseline", "a3_workaware_on", "dcs_joint_on", "dcs_a3_minimal_on"}:
    ...
    a3_loop_enabled = policy_enabled or arm == "dcs_a3_minimal_on"
    admission_policy_enabled = arm in {"a3_workaware_on", "dcs_joint_on"}
    if not a3_loop_enabled:
        ...
        if not policy_enabled and policy_value not in DISABLED_VALUES:
            raise ContractError("RELAX_SYNC_INTENT_POLICY must be disabled for minimal A3")
        ...
        if admission_policy_enabled:
            ...
            if forbidden_admission_env:
                raise ContractError(f"minimal A3 admission environment must be absent: {forbidden_admission_env}")
    ...
    return {
        ...
        "cross_version_kv_continuation": a3_loop_enabled,
        "priority_scheduling": admission_policy_enabled,
        "slime_router_work_aware": admission_policy_enabled,
        "slime_router_default_least_loaded": arm == "dcs_a3_minimal_on",
        "hybrid_dcs_weight_sync": arm in {"dcs_joint_on", "dcs_a3_minimal_on"},
        "hybrid_weights_backuper_on_gpu": arm in {"dcs_joint_on", "dcs_a3_minimal_on"},
        "targeted_retirement": arm in {"dcs_joint_on", "dcs_a3_minimal_on"},
