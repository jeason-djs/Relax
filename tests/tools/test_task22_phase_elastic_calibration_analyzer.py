...
def _gate_lines(*, gate_walls: dict[int, float] | None = None) -> list[str]:
    walls = gate_walls or {}
    return [
        (
            "prefix TASK22_CALIBRATION_GATE "
            f"logical_step={step} sync_id={step + 1} "
            f"t_begin={100.0 + (step - 5) * 10:.6f} "
            f"t_end={100.0 + (step - 5) * 10 + walls.get(step, 4.0):.6f} "
...
def _actor_lines(*, gate_walls: dict[int, float] | None = None) -> list[str]:
    walls = gate_walls or {}
    lines = []
    for step in range(5, 15):
        gate_begin = 100.0 + (step - 5) * 10
...
def test_analyzer_rejects_unsuccessful_or_mismatched_run_contract(tmp_path, defect) -> None:
    ...
    if defect == "lifecycle":
        ...
        contract["workload"]["num_rollout"] = 15
    else:
...
def test_analyzer_excludes_terminal_local_exception_from_timing_requirement(tmp_path) -> None:
    ...
    assert timing["terminal_with_exception_excluded_count"] == 1
