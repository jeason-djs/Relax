...
class Rollout(Base):
    ...
    def __init__(
        ...
        self._task22_sync_intent_id: int | None = None
    ...
    @app.post("/task22_sync_intent")
    async def task22_sync_intent(self, sync_id: int, actor_rollout_id: int):
        result = await self.rollout_manager.begin_task22_sync_intent.remote(sync_id, actor_rollout_id)
        if result.get("enabled"):
            self._task22_sync_intent_id = sync_id
        return result

    @app.get("/task22_sync_intent")
    async def task22_sync_intent_status(self):
        return await self.rollout_manager.get_task22_sync_intent.remote()
    ...
    async def end_update_weight(self):
        ...
        try:
            ...
            if self._task22_sync_intent_id is not None:
                await self.rollout_manager.end_task22_sync_intent.remote(self._task22_sync_intent_id)
                self._task22_sync_intent_id = None
