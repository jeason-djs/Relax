...
def _task22_calibration_actor_phase(rollout_id: int, phase: str, begin: float, end: float | None = None) -> None:
    """Emit rank-zero absolute timing only for the opt-in calibration run."""
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def train_hybrid(self, rollout_id) -> None:
        ...
        with inverse_timer("train_wait"), timer("train"):
            ...
            if self.args.use_routing_replay:
                os.environ["ROUTING_REPLAY_STAGE"] = "replay_backward"
            actor_train_begin = time.time()
            _task22_calibration_actor_phase(rollout_id, "actor_train_begin", actor_train_begin)
            with timer("actor_train"):
                train(
                    rollout_id,
                    self.model,
            ...
            _task22_calibration_actor_phase(rollout_id, "actor_train", actor_train_begin, time.time())
