...
async def generate(
    ...
    # The opt-in calibration uses a unique request id to join client permit
    # lifecycle rows with prompt-free SGLang scheduler snapshots.  A unique rid
    # does not alter routing or sampling behavior.
    sample_metadata = getattr(sample, "metadata", None)
    if permit_observability_dir() is not None and isinstance(sample_metadata, dict):
        calibration_rid = sample_metadata.get(CALIBRATION_RID_METADATA_KEY)
