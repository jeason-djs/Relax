...
"""Opt-in, prompt-free observability for inference-permit calibration.

The feature is enabled only when ``TASK22_CALIBRATION_DIR`` is set.  It does
not replace or wrap the production semaphore and records no prompt text or
token ids.
"""
...
def begin_wait(
    ...
    *,
    ...
    return {
        ...
        # Fresh samples are tokenized inside ``generate`` and can still have an
        # empty ``tokens`` list here.  ``mark_terminal`` fills the exact value
        # from the post-request sample when possible.
...
def capture_sglang_timing(sample: Any, meta_info: dict[str, Any]) -> None:
    """Keep only finite, prompt-free SGLang request timing fields until
    terminal export."""
...
def mark_terminal(row: dict[str, Any], sample: Any, error: BaseException | None = None) -> None:
    ...
    metadata = getattr(sample, "metadata", None)
    timing = metadata.pop(CALIBRATION_TIMING_METADATA_KEY, {}) if isinstance(metadata, dict) else {}
    ...
    row.update(
        {
            ...
            "sglang_forward_entry_epoch": forward_epoch
            if isinstance(forward_epoch, float) and forward_epoch > 0
            else None,
...
def export_rows(rows: list[dict[str, Any]], *, directory: str, physical_rollout_id: int) -> Path:
    """Atomically publish a complete per-physical-rollout JSONL file."""
